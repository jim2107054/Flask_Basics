...
def user(name):
    return f"<h1>Hello, {name.title()}!</h1><p>Welcome to your user page.</p>"
...
@app.route('/user/<name>/<int:age>/<float:height>') #we can also specify multiple parameters
def user_with_age_and_height(name, age, height):
    return f"<h1>Hello, {name.title()}!</h1><p>You are {age} years old and {height} feets tall.</p>"


#state the application
# to run the app, use the command: flask run
if __name__ == '__main__': # This application will run only if we are in app.py module. To avoid any import issues, we use this condition.
